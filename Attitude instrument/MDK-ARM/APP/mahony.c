...
//  float magX, magY;
  ...
  initialRoll = atan2f(acc_data.y, acc_data.z);//根据加速度算角度
  initialPitch = -atan2f(acc_data.x, acc_data.z);
  
//	initialRoll = 0;//根据加速度算角度
//  initialPitch = 0;
...
/*
参考资料：https://www.bilibili.com/read/cv23893314/
*/
...
{
//    attitude_roll  = (atan2f(rMat[2][1], rMat[2][2])-initialRoll)*SEC2DEG;
//    attitude_pitch = (-asinf(rMat[2][0])-initialPitch)*SEC2DEG;
	  attitude_roll  = atan2f(rMat[2][1], rMat[2][2])*SEC2DEG;
    attitude_pitch = -asinf(rMat[2][0])*SEC2DEG;
    attitude_yaw   = atan2f(rMat[1][0],rMat[0][0])*SEC2DEG;
}
...
void imuCalculateEstimatedAttitude(void)
{
    ...
    uint8_t useYaw = 0;
		...
		navi_prepare_imu_data();/*得到陀螺仪原始数据并转换为实际物理单位*/
		
		if(!imu_offset_flag)/* 计算陀螺仪零飘 */
		{
		...
		if(!imu_init_flag)
    {
      ...
      return;
		...
		if(imu_offset_flag && imu_init_flag)
		{
			imuMahonyAHRSupdate(0.01,
                        imu_data.gx, imu_data.gy, imu_data.gz,
                        useAcc, imu_data.ax, imu_data.ay,imu_data.az, 
                        useMag, 0, 0, 0,
                        useYaw, 0);
...
//		attitude_roll = atan2f(acc_data.y, acc_data.z);//根据加速度算角度
//		attitude_pitch = -atan2f(acc_data.x, acc_data.z);
//  
